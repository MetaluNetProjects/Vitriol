...
void Flock_attractPt(t_boids *x, t_float num, t_float px, t_float py);
...
void boids2d_setup(void)
{
    ...
    class_addmethod(boids2d_class, (t_method) Flock_attractPt,    gensym("attractpt"), A_FLOAT, A_FLOAT, A_FLOAT, 0);
...
void FlightStep(t_boids *x)
{
    ...
    for (i = 0; i < x->numBoids; i++) {
        ...
        goAttract.x = goAttract.y = 0.0;
        
        goAttractVel = SeekPoint(x, i, x->attractPt[0]);
